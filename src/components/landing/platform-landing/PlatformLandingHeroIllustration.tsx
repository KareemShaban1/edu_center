...
import { PLATFORM_LANDING_HERO_IMAGE_KEY } from '@/config/website-images';
import { useWebsiteImages } from '@/contexts/WebsiteImagesContext';
import { useLandingBrand } from './useLandingBrand';

/** Laptop + phone mockup matching the marketing design (default when no upload override). */
export function PlatformLandingHeroIllustrationDefault({ className }: { className?: string }) {
...
/** Uses developer image override when set; otherwise the built-in illustration. */
export function PlatformLandingHeroIllustration({ className }: { className?: string }) {
  const { resolveByKey } = useWebsiteImages();
  const overrideUrl = resolveByKey(PLATFORM_LANDING_HERO_IMAGE_KEY);
  ...
  return <PlatformLandingHeroIllustrationDefault className={className} />;
}
