...
export default function AdminQuizForm() {
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      adminAssessmentsApi.saveQuizDate(
        Number(sectionId),
        currentDate,
  ...
  const updateRow = (studentId: number, field: keyof StudentQuizRow, value: string) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      toast({
        title: t('quiz.saved'),
        description: `${t('quiz.savedDesc')} ${currentDate}`,
      });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div className="flex items-center gap-3">
          <Button asChild variant="ghost" size="icon">
            <Link to={isToday ? '/admin/quizzes' : `/admin/quizzes/section/${sectionId}/history`}>
              <ArrowLeft className="h-4 w-4" />
            </Link>
          </Button>
          <div>
            <h1 className="page-title">
              {t('nav.quizzes')} — {isToday ? t('attendance.today') : currentDate}
            </h1>
