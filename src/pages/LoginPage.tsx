...
import { GraduationCap, Shield, BookOpen, Users, Globe, Languages } from 'lucide-react';
...
const isPlatformGuard = (guard: string) => guard === 'super_admin' || guard === 'platform_admin';
...
export default function LoginPage() {
  ...
  const [email, setEmail] = useState('admin@educenter.com');
  const [password, setPassword] = useState('password');
  const [tenantSlug, setTenantSlug] = useState('demo');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);

  const { data: guards = ['users', 'teacher', 'parent', 'student', 'super_admin'] } = useQuery({
    queryKey: ['auth-guards'],
    queryFn: () => authApi.getGuards(),
  });

  useEffect(() => {
    if (guards.length > 0 && !guards.includes(selectedGuard)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const user = await login(email, password, selectedGuard, isPlatformGuard(selectedGuard) ? undefined : tenantSlug);
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-elevated">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!isPlatformGuard(selectedGuard) && (
              <div>
                <label htmlFor="tenant" className="mb-1.5 block text-sm font-medium">{t('auth.tenantCode')}</label>
                <input
                  id="tenant"
                  type="text"
                  value={tenantSlug}
