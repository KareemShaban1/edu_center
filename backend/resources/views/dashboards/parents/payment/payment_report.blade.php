...
@section('title', trans('Parents_Dashboard/reports_trans.Payment_Report'))
...
<div class="container mx-auto p-4">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <!-- Results -->
            <div id="paymentResults" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 ">
                        ...
                        <div class="bg-white rounded-lg shadow hover:shadow-xl transition transform hover:-translate-y-1 hover:scale-105 p-5 flex flex-col justify-between duration-300 animate-fade-in">
                            <div>
                                ...
                                <p class="text-sm text-gray-600">
                                    <span class="font-semibold">{{ trans('Parents_Dashboard/reports_trans.Payment_Date') }}:</span>
                            ...
                            <div class="mt-4">
                                <p class="text-lg font-bold text-blue-600">
                                    {{ trans('Parents_Dashboard/reports_trans.Amount') }}: {{ $payment->amount }}
                                </p>
                                <p class="text-lg font-bold text-gray-600">
                                    {{ trans('Parents_Dashboard/reports_trans.Payment_Status') }}: 
                                    @if ($payment->payment_status == 'paid')
                                       <span class="text-green-600"> {{ trans('Parents_Dashboard/reports_trans.Paid') }}</span>
                                    @else
                                        <span class="text-red-600"> {{ trans('Parents_Dashboard/reports_trans.Not_Paid') }}</span>
                                    @endif
                                </p>
                                @if ($payment->notes)
                                    <p class="text-sm text-gray-500 mt-1">
                                        {{ trans('Parents_Dashboard/reports_trans.Notes') }}: {{ $payment->notes }}
                                    </p>
                                @endif
                            </div>
                        </div>
                    @empty
                        <div class="col-span-full text-center text-gray-500">
                            {{ trans('Parents_Dashboard/reports_trans.No_payments_Found') }}
                        </div>
                    @endforelse
                </div>
            </div>
...
$(document).ready(function () {
    $('#filterBtn').on('click', function () {
        ...
        $.ajax({
            url: "{{ route('sons.payment_report') }}",
            method: "GET",
            data: { start_date, end_date, month_year, student_id },
            success: function (response) {
                let payments = response.payments;
                let container = $('#paymentResults');
                container.empty();

                if (payments.length === 0) {
                    container.append('<p class="text-red-500 text-center">{{ trans("Parents_Dashboard/reports_trans.No_payments_Found") }}</p>');
                    return;
                }

                let grid = $('<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6"></div>');

                payments.forEach((payment) => {
                    grid.append(`
                        <div class="bg-white rounded-lg shadow hover:shadow-xl transition transform hover:-translate-y-1 hover:scale-105 p-5 flex flex-col justify-between duration-300 animate-fade-in">
                            <div>
                                <h3 class="text-lg font-bold text-gray-800 mb-2">${payment.student.name}</h3>
                                <p class="text-sm text-gray-600">
                                    <span class="font-semibold">{{ trans('Parents_Dashboard/reports_trans.payment_Date') }}:</span> ${payment.payment_date}
                                </p>
                                <p class="text-sm text-yellow-600">
                                    ${payment.month}
                                </p>
