...
import LocationSelectFields, { locationPayloadFromValues, locationValuesFromSettings } from '@/components/LocationSelectFields';
...
type SettingsForm = AdminSettings & ReturnType<typeof locationValuesFromSettings>;

const emptyLocation = locationValuesFromSettings({});

export default function AdminSettings() {
  const { t } = useLocale();
  const queryClient = useQueryClient();
  const [form, setForm] = useState<SettingsForm>({ ...emptyForm, ...emptyLocation });
  ...
  useEffect(() => {
    if (data) setForm({ ...data, ...locationValuesFromSettings(data) });
  }, [data]);

  const saveMutation = useMutation({
    mutationFn: (payload: AdminSettings & { generate_now?: boolean }) => adminSettingsApi.update(payload),
    onSuccess: async result => {
      setForm({ ...result.settings, ...locationValuesFromSettings(result.settings) });
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    saveMutation.mutate({
      ...form,
      ...locationPayloadFromValues(form),
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-2 xl:items-start">
          ...
          <div className="space-y-6">
            <div className="rounded-xl border border-border bg-card p-6 shadow-card space-y-4">
              <h3 className="font-display font-semibold">{t('settings.mapPicker')}</h3>
              <LocationMapPicker
                lat={form.lat}
                long={form.long}
                onChange={(nextLat, nextLong) => setForm(f => ({ ...f, lat: nextLat, long: nextLong }))}
              />
            </div>
