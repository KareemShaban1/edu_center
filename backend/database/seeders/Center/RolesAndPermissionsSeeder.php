    ...
    public function run()
    {
        ...
        foreach ($permissions as $permission) {
            ...
            if (CenterContext::hasCenter()) {
                $lookup['center_id'] = CenterContext::id();
                $permission['center_id'] = CenterContext::id();
            }
            Permission::firstOrCreate($lookup, $permission);
        }
        
        
        // Create roles and assign existing permissions
        $userRoleLookup = ['name' => 'user', 'guard_name' => 'web'];
        $adminRoleLookup = ['name' => 'admin', 'guard_name' => 'web'];
        if (CenterContext::hasCenter()) {
            $userRoleLookup['center_id'] = CenterContext::id();
            $adminRoleLookup['center_id'] = CenterContext::id();
        }

        $userRole = Role::firstOrCreate($userRoleLookup, $userRoleLookup);
        $userRole->syncPermissions(['create user']);

        $adminRole = Role::firstOrCreate($adminRoleLookup, $adminRoleLookup);
        $permissionQuery = Permission::where('guard_name', 'web');
        if (CenterContext::hasCenter()) {
            $permissionQuery->where('center_id', CenterContext::id());
        }
