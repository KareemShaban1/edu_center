...
function AnnouncementForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!form.title.trim() || !form.content.trim() || !form.grade_id || !form.class_id || !form.section_id) {
      toast({ title: 'Validation error', description: 'Please fill required announcement fields.', variant: 'destructive' });
      return;
    }
    onSave(
      {
  ...
  return (
    <FormDialog open title={item ? t('crud.edit') : t('crud.addNew')} onClose={onClose} onSubmit={handleSubmit} loading={saving}>
            ...
            {item.media.map(m => {
              ...
              return (
                <div key={m.id} className={`flex items-center justify-between rounded-md px-2 py-1.5 ${marked ? 'bg-destructive/10' : 'bg-muted/30'}`}>
                  ...
                  <button type="button" onClick={() => toggleRemoveMedia(m.id)} className={`rounded p-1 ${marked ? 'text-destructive' : 'text-muted-foreground hover:text-foreground'}`}>
      ...
      <FormField label="Upload Files (multiple media types)" id="ann-files">
        <input
          id="ann-files"
          title="Announcement files"
          aria-label="Announcement files"
          type="file"
          multiple
...
export default function AdminAnnouncements() {
  ...
  const columns: CrudColumn<AnnouncementItemPayload>[] = [
    { key: 'id', label: t('col.id'), sortable: true },
    { key: 'title', label: t('col.title'), sortable: true },
    { key: 'content', label: t('col.content'), render: a => <span className="line-clamp-1 max-w-xs">{a.content}</span> },
    { key: 'type', label: t('col.type'), render: a => <span className="capitalize">{a.type}</span> },
    ...
    { key: 'created_at', label: t('col.date'), sortable: true },
    {
      ...
      label: t('crud.view'),
      render: a => (
        <button onClick={() => setViewItem(a)} className="rounded-lg p-1.5 hover:bg-muted text-muted-foreground hover:text-foreground transition-colors" aria-label={t('crud.view')}>
  ...
  return (
    <>
      <CrudPage<AnnouncementItemPayload>
        title={t('nav.announcements')}
        description={t('page.announcements.desc')}
        columns={columns}
        data={announcements}
        searchKeys={['title', 'content', 'type', 'grade_name', 'class_name', 'section_name']}
