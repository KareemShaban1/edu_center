...
**EduCenter** is a multi-tenant **education management platform** for schools and training centers. Each tenant (center/school) gets an isolated database and domain-aware routing. The product supports **English** and **Arabic** with **RTL** layout switching via `LocaleContext`.

The system targets **administrators, teachers, students, parents**, and a separate **platform** tier for **super admins / platform operators** who manage tenants, subscriptions, and global access.
...
- **Auth**: JWT (or equivalent) stored via `apiClient`; guards (`users`, `teacher`, `parent`, `student`, `super_admin`, `platform_admin`) drive login and role normalization.
- **Tenant context**: `tenantSlug` / `tenantId` on login for non-platform guards; platform logins omit tenant.
...
| Central DB | Tenants, domains, platform admins, subscriptions metadata |
| Tenant DB | Students, teachers, parents, grades, classes, sections, attendance, fees, exams, quizzes, homework, library, announcements, meeting series, etc. |

- **API** routes exposed for SPA (`/api/...`) with tenant resolution from host/slug/header per deployment.
...
- SPA static assets from `dist/`; **API** must not be swallowed by SPA `try_files` — see `docs/DEPLOYMENT_CONTABO_AAPANEL.md` for nginx `/api` FastCGI pattern.
- MySQL grants for tenant DB creation/migrations are documented for multi-DB tenancy.
