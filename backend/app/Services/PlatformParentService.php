...
final class PlatformParentService
{
    ...
    public function list(string $conn): Collection
    {
        ...
        $membershipsByUser = $this->membershipRepository->membershipsByUser($conn, Parents::class);
        $childrenCounts = $this->childrenCounts($conn);

        return DB::connection($conn)->table('parents')
            ->orderByDesc('id')
            ->get()
    ...
    public function show(string $conn, int $id): ?array
    {
        ...
        $centers = $this->membershipRepository->membershipsByUser($conn, Parents::class)->get($id, collect())->values()->all();
