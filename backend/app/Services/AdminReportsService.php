...
final class AdminReportsService
{
     ...
     */
    public function index(Connection $tenantDb): array
    {
     ...
     * @return array<string, mixed>
     */
    public function show(Connection $tenantDb, Request $request, string $type): array
    {
        ...
        if ($type === 'attendance') {
            return $this->attendanceReport($tenantDb);
        }

        if ($type === 'exams' || $type === 'quizzes') {
            return $this->degreeReport($tenantDb, $type);
        }

        return $this->paymentsReportService->build($request, $tenantDb);
    }

    /**
     * @return array<string, mixed>
     */
    private function attendanceReport(Connection $tenantDb): array
    {
        $hasTable = fn (string $table) => Schema::connection('center')->hasTable($table);
     ...
     * @return array<string, mixed>
     */
    private function degreeReport(Connection $tenantDb, string $type): array
    {
        $hasTable = fn (string $table) => Schema::connection('center')->hasTable($table);
