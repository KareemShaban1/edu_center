...
import { teacherSessionsApi } from '@/services/endpoints/teacher-sessions';

export default function TeacherLiveKitSession() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { token, url } = await teacherSessionsApi.getLiveKitToken(id);
        if (!url || !token) {
    ...
    <div className="mx-auto max-w-4xl space-y-4 p-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <Button variant="outline" asChild>
          <Link to="/teacher/sessions">{t('misc.back')}</Link>
        </Button>
      </div>
